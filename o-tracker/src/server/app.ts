...
export async function buildServer(bot: Bot): Promise<FastifyInstance> {
  ...
  app.get<{ Querystring: Record<string, string> }>("/api/dashboard", async (req, reply) => {
    ...
    // Refresh from Oura so the dashboard matches the app — but at most once per
    // 10 minutes per user, to avoid hammering the Oura API on every refresh.
    if (user.ouraConnected) {
      const last = await prisma.dailyMetric.findFirst({
        where: { userId: user.id },
        orderBy: { day: "desc" },
        select: { updatedAt: true },
      });
      const stale = !last || Date.now() - last.updatedAt.getTime() > 10 * 60 * 1000;
      if (stale) await syncUser(user.id, 2).catch((e) => req.log.error(e));
    }
